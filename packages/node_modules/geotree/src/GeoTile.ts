...
import { StringRef } from 'charto-clip';
...
export class GeoTile extends QuadTile implements BBoxLike, TileGeometry {
	...
	/** @param outer Child tile to check.
	  * @param inner Corner of child tile.
	  * @param x1 Line segment start coordinate.
	  * @param y1 Line segment start coordinate.
	  * @param x2 Line segment end coordinate.
	  * @param y2 Line segment end coordinate.
	  * @return Arbitrary number, sign indicates side of child tile corner
	  * relative to the line segment. */

	private testCorner(outer: QuadPos, inner: QuadPos, x1: number, y1: number, x2: number, y2: number) {
		/** Equals this.s, this.n or their average. */
		const x = this.n - ((outer + 1 & 2) + (inner + 1 & 2)) * ((this.n - this.s) / 4);
		/** Equals this.w, this.e or their average. */
		const y = this.w + ((outer & 2) + (inner & 2)) * ((this.e - this.w) / 4);
	...
	private insertCurveLeaf(
		...
		const shapeList = isSurface ? this.surfaceList : this.curveList;
		let refList: StringRef[];

		// Append curve to the latest shape if belonging to it,
		// otherwise insert a new shape.

		if(this.latestShape == shape) {
			refList = shapeList[shapeList.length - 1];
		} else {
			shapeList.push(refList = []);
			this.latestShape = shape;
		}

		refList.push({ curve: geom, first, last });
	...
	private insertCurveAdjacent(
		...
		if(
			...
			this.childList![pos]!.insertCurveCrossing(geom, first, last, isSurface, shape, touchedTileList);
		}
	}
	...
	private insertCurveCrossing(
		...
		if(!childList) {
			return(this.insertCurveLeaf(geom, first, last, isSurface, shape, touchedTileList));
		}

		for(let child of childList) {
			if(child!.intersectsLine(geom.x[first], geom.y[first], geom.x[last], geom.y[last])) {
				child!.insertCurveCrossing(geom, first, last, isSurface, shape, touchedTileList);
			}
		}
	}
	...
	/** Insert a polyline into this tile, split so that children will contain
	  ...
	  *
	  * This algorithm is optimized for inserting complicated polylines.
	  * Traversing up and down quadtree nodes between adjacent points is minimized.
	  *
	...
	insertCurve(
		...
		do {
			...
			if(num > first || (num == first && first > 0)) {
				...
				if((pos ^ posPrev!) == 3) {
					...
					childList[posNext]!.insertCurveCrossing(geom, num - 1, num, isSurface, shape, touchedTileList);
				}
				...
				if(num == first && outside && pos != posPrev) {
					this.insertCurveAdjacent(geom, num, num - 1, pos, posPrev!, -sign, isSurface, shape, touchedTileList);
				}
			}
			...
			if(num < first) {
				...
				// same quadrant).
				num = childList[pos]!.insertCurve(geom, num, last, isSurface, shape, touchedTileList);
			}
		} while(num <= last);
		...
		if(outside && pos != posPrev) {
			this.insertCurveAdjacent(geom, num - 1, num, posPrev!, pos, sign, isSurface, shape, touchedTileList);
		}
	...
	/** Latest parent shape, to detect additional parts inserted immediately after. */
	latestShape?: cgeo.Curve | cgeo.Surface;
