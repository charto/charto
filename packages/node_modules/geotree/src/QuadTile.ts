...
import { BBoxLike } from 'charto-types';
...
export interface TileClass<Tile extends QuadTile = QuadTile> {
	new(s: number, w: number, n: number, e: number, tree: QuadTree, path?: string): Tile
}

export class QuadTile implements BBoxLike {
	...
	makeChild(
		...
		return(new Tile(s, w, n, e, this.tree, this.path + pos));
	}
