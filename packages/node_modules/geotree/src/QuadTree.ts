...
export class QuadTree<Tile extends QuadTile = QuadTile> implements QuadTreeOptions {

	constructor(
		options: QuadTreeOptions,
		public Tile = QuadTile as TileClass<Tile>
	) {
		const { s, w, n, e } = options;
		this.s = s;
		this.w = w;
		this.n = n;
		this.e = e;
		this.root = new Tile(s, w, n, e, this);
	}
	...
	/** Return up to 4 tiles of roughly similar size covering the query box,
	  * at least one overlapping by at least a quarter of the tile's size.
	  * @param split Split tiles as needed to match the desired size. */

	getCover(s: number, w: number, n: number, e: number, split?: boolean) {
		const result: Tile[] = [];
		...
		this.iterate((tile: Tile) => {
			...
			if(nsCoverage > 0 && ewCoverage > 0) {
				...
				if(
					...
					// ...then split the tile.
					if(!tile.childList && split) tile.split();
				} else {
					result.push(tile);
					return(false);
				}
