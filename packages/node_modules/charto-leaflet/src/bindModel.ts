...
export function bindModel(map: L.Map, model: MapModel) {
	...
	function updateModel(e: L.LeafletEvent) {
		...
		if(!model.boxWidth || !model.boxHeight) {
			if(
				model.sw.x != sw.lat || model.sw.y != sw.lng ||
				model.ne.x != ne.lat || model.ne.y != ne.lng
			) {
				model.setBounds(sw.lat, sw.lng, ne.lat, ne.lng);
			}
		}

		if(model.center.x != ll.lat || model.center.y != ll.lng || model.zoom != zoom) {
			model.setCenterZoom(ll.lat, ll.lng, zoom);
		}
	}
