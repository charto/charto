...
import { Layer, LayerFeatures, LayerFeaturesPromise } from 'charto-model';
import { CanvasRenderer, GLRenderer } from 'charto-render';
...
export class CanvasLayer extends L.GridLayer {
	...
	constructor(public layer: Layer, public renderer: CanvasRenderer | GLRenderer) {
		super();
	}
	...
	createTile(xyz: PointZ, cb: (err: any, canvas: HTMLCanvasElement) => void) {
		...
		new Promise((resolve: (layerFeatures: LayerFeaturesPromise) => void) => {
			...
			return(this.renderer.render({
				...
				features,
				bbox,
				widthPixels: size.x * pixelRatio,
				heightPixels: size.y * pixelRatio,
				pixelRatio,
				zoom
			}));
		}).catch(
