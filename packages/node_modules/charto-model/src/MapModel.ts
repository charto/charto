...
export const ShopLayerData = Layer.props({
	counts: T.optional(FeatureCountModel, {}),
	price: -1
});
...
export const DataModel = T.model({
	'akaava:Kaava': ShopLayer,
	'kanta:Kivi': ShopLayer
});
...
export class MapModelCode extends shim(MapModelData) {
	...
	setCenter(x: number, y: number) {
		...
		applySnapshot(this.data, {
			'akaava:Kaava': {
				counts: {
					'2017-091': ~~(Math.random() * 100),
					'2017-564': ~~(Math.random() * 100)
				},
				visible: true,
				price: 0.1
			},
			'kanta:Kivi': {
				counts: {
					'2017-091': ~~(Math.random() * 1000),
					'2017-564': ~~(Math.random() * 1000)
				},
				visible: false,
				price: 0.01
			}
