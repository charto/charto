...
export const MapModelData = T.model({
	...
	// View corners (or box corners if boxWidth and boxHeight are >0)
...
export class MapModelCode extends shim(MapModelData) {
		...
		applySnapshot(this.layers, [
			{
				name: 'akaava:Kaava',
				counts: [
					{ region: '2017-091', count: ~~(Math.random() * 100) },
					{ region: '2017-564', count: ~~(Math.random() * 100) }
				],
				visible: true,
				price: 0.1
			}, {
				name: 'kanta:Kivi',
				counts: [
					{ region: '2017-091', count: ~~(Math.random() * 1000) },
					{ region: '2017-564', count: ~~(Math.random() * 1000) }
				],
				visible: false,
				price: 0.01
			}
	...
	@action
	drawLightBox(width: number, height: number) {
		this.boxWidth = width;
