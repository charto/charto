...
import { ElementToken, TokenElement } from 'cxml';
...
export class GeometryCollection extends cgeo.GeometryCollection<(Geometry & cgeo.Geometry) | null> {
	...
	writeGML(state: WriteState, childElement?: TokenElement) {
		...
		for(let child of this.childList) {
			if(child) child.writeGML(state, childElement);
		}
	...
	static finishGML(parser: Parser<any>, token: ElementToken): GeometryCollection | null {
		...
		const TypeHandler = parser.gmlConfig.geometryTbl[token.id!] as typeof cgeo.GeometryCollection;
