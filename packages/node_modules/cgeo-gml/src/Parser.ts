...
export class Parser<WrapperToken extends cxml.Token> extends stream.Transform {

	constructor(
		public Wrapper: { new(geometry: cgeo.Geometry): WrapperToken },
		public config: ConfigGML
	) {
	...
	_transform(chunk: cxml.TokenChunk | null, enc: string, flush: (err: any, chunk: cxml.TokenChunk | null) => void) {
		...
		const { gml, geometryTbl, tokenTbl } = this.config;
