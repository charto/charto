...
const gmlConfig = new ConfigGML({ config });

const writer1 = new cxml.Writer(config);
const writer2 = new cxml.Writer(config, { gmlConfig });

config.bindNamespace(new cxml.Namespace('gml', 'http://www.opengis.net/gml'));
...
writer1.pipe(config.createParser()).pipe(new Parser(GeometryToken, gmlConfig)).pipe(writer2);
// writer1.pipe(process.stdout);
