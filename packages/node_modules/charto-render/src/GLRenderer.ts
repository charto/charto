...
export class GLRenderer {
	...
	render(options: GLRendererOptions) {
		...
		const bufferSize = Math.min(4096, pointCount);
		...
		if(pointCount > bufferSize) {
			...
			for(let geom of pointList) {
				...
				if(!index) {
					index = ++p2;
					// TODO: Move these transforms to the shader.
		...
		const [ uSize, uEdge, uAlpha, uColor ] = this.shader.getUniformLocations([
			'uSize', 'uEdge', 'uAlpha', 'uColor'
		]);
