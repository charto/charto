...
export class CanvasRendererState extends RendererState {
	...
	constructor(options: RendererOptions) {
		...
		this.scaleX = options.widthPixels / (e - w);
		this.scaleY = options.heightPixels / (s - n);
		this.offsetX = -w;
		this.offsetY = -n;
	}
...
export class CanvasRenderer {
	...
	renderPoint(geom: Point, state: CanvasRendererState) {
		const x = (geom.y + state.offsetX) * state.scaleX;
		const y = (geom.x + state.offsetY) * state.scaleY;
		const size = state.pointRadius * 2;
		let weight = geom.weight || 1;
	...
	render(options: CanvasRendererOptions) {
		...
		const data = gc.getImageData(0, 0, options.widthPixels, options.heightPixels);
		const buf = data.data;
		const len = options.widthPixels * options.heightPixels * 4;
